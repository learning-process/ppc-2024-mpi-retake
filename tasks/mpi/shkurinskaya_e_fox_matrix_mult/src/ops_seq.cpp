  ...
  for (int it = 0; it < root_; ++it) {
    ...
    if (world_.rank() == 0) {
      for (int i = 0; i < block_sz_; ++i) {
        for (int j = 0; j < block_sz_; ++j) {
          for (int k = 0; k < block_sz_; ++k) {
            temp_output[(i * sz_) + j] += left_block[(i * block_sz_) + k] * right_block[(k * block_sz_) + j];
          }
        }
      }
    } else {
      for (int i = 0; i < block_sz_; ++i) {
        for (int j = 0; j < block_sz_; ++j) {
          for (int k = 0; k < block_sz_; ++k) {
            local_res[(i * block_sz_) + j] += left_block[(i * block_sz_) + k] * right_block[(k * block_sz_) + j];
          }
        }
      }
    }
...
bool shkurinskaya_e_fox_mat_mul_mpi::FoxMatMulMPI::PostProcessingImpl() {
  if (world_.rank() == 0) {
    ...
    std::copy(output_.begin(), output_.end(), it1);
  }
  return true;
}
