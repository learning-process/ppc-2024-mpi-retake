...
bool malyshev_v_conjugate_gradient_mpi::ConjugateGradientMethod::PreProcessingImpl() {
  if (world_.rank() == 0) {
...
bool malyshev_v_conjugate_gradient_mpi::ConjugateGradientMethod::ValidationImpl() {
  if (world_.rank() == 0) {
...
bool malyshev_v_conjugate_gradient_mpi::ConjugateGradientMethod::RunImpl() {
  ...
  while (true) {
    double local_rsquare = malyshev_v_conjugate_gradient_mpi::ScalarProduct(local_r, local_r);
    ...
    double local_p_ap = malyshev_v_conjugate_gradient_mpi::ScalarProduct(local_p, local_ap);
    double p_ap = 0.0;
    ...
    local_rsquare = malyshev_v_conjugate_gradient_mpi::ScalarProduct(local_r, local_r);
    rsquare_k = 0.0;
    boost::mpi::reduce(world_, local_rsquare, rsquare_k, std::plus<>(), 0);
    boost::mpi::broadcast(world_, rsquare_k, 0);
...
bool malyshev_v_conjugate_gradient_mpi::ConjugateGradientMethod::PostProcessingImpl() {
  if (world_.rank() == 0) {
...
bool malyshev_v_conjugate_gradient_mpi::IsPositiveDefinite(const std::vector<double>& mat, size_t size) {
...
bool malyshev_v_conjugate_gradient_mpi::IsSymmetric(const std::vector<double>& mat, size_t size) {
  for (size_t i = 0; i < size; i++) {
...
double malyshev_v_conjugate_gradient_mpi::ScalarProduct(const std::vector<double>& a, const std::vector<double>& b) {
  double result = 0.0;
