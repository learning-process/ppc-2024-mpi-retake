...
bool deryabin_m_cannons_algorithm_mpi::CannonsAlgorithmMPITaskSequential::PreProcessingImpl() {
  ...
  input_matrix_B = std::vector<double>(task_data->inputs_count[1]);
  auto* tmp_ptr_a = reinterpret_cast<double*>(task_data->inputs[0]);
  auto* tmp_ptr_b = reinterpret_cast<double*>(task_data->inputs[1]);
  std::copy(tmp_ptr_a, tmp_ptr_a + task_data->inputs_count[0], input_matrix_A.begin());
  std::copy(tmp_ptr_b, tmp_ptr_b + task_data->inputs_count[1], input_matrix_B.begin());
...
bool deryabin_m_cannons_algorithm_mpi::CannonsAlgorithmMPITaskSequential::RunImpl() {
  ...
  auto dimension = sqrt(static_cast<unsigned short>(input_matrix_A.size()));
  while (i != dimension) {
    j = 0;
    while (j != dimension) {
      count = 0;
...
bool deryabin_m_cannons_algorithm_mpi::CannonsAlgorithmMPITaskSequential::PostProcessingImpl() {
  reinterpret_cast<std::vector<double>*>(task_data->outputs[0])[0] = output_matrix_C;
  return true;
}
...
bool deryabin_m_cannons_algorithm_mpi::CannonsAlgorithmMPITaskParallel::RunImpl() {
  ...
  if (world.size() == 1 || world.size() != pow(sqrt(static_cast<unsigned short>(world.size())), 2) ||
      sqrt(static_cast<unsigned short>(input_matrix_A.size())) % sqrt(static_cast<unsigned short>(world.size())) != 0) {
    if (world.rank() == 0) {
      dimension = sqrt(static_cast<unsigned short>(input_matrix_A.size()));
