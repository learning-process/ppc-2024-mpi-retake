  ...
  if (color_ == 1) {
    ...
    if (comm_.rank() == 0) {
      ...
      boost::mpi::gatherv(comm_, loc_res_val_.data(), static_cast<int>(loc_res_val_.size()), res_val_.data(), sizes_val,
                          0);  // NOLINT
      boost::mpi::gatherv(comm_, loc_res_row_ind_.data(), static_cast<int>(loc_res_row_ind_.size()), res_ind_.data(),
                          sizes_val,
                          0);  // NOLINT
      boost::mpi::gatherv(comm_, loc_res_col_ptr_.data(), static_cast<int>(loc_res_col_ptr_.size() - 1),
                          res_ptr_.data(), sizes_ptr,
                          0);  // NOLINT
