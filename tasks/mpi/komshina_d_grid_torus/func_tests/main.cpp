...
TEST(komshina_d_grid_torus_mpi, identical_source_and_target) {
  ...
  komshina_d_grid_torus_mpi::TestTaskMPI::TaskData in("test data", 0);
  ...
  if (world.rank() == 0) {
    ...
    ASSERT_EQ(out.path, std::vector<int>{0});
  }
}

TEST(komshina_d_grid_torus_mpi, target_out_of_bounds) {
  boost::mpi::communicator world;
  if (world.size() < 4) {
    GTEST_SKIP();
    return;
  ...
  komshina_d_grid_torus_mpi::TestTaskMPI::TaskData in("invalid", 999999);
  komshina_d_grid_torus_mpi::TestTaskMPI::TaskData data_out_struct;
...
TEST(komshina_d_grid_torus_mpi, random_target_process) {
  boost::mpi::communicator world;
  if (world.size() < 4) {
    GTEST_SKIP();
    return;
  ...
  int random_dest = dist(gen);

  komshina_d_grid_torus_mpi::TestTaskMPI::TaskData in("random test", random_dest);
  komshina_d_grid_torus_mpi::TestTaskMPI::TaskData out;
  ...
  if (world.rank() == 0) {
    ASSERT_EQ(out.payload, in.payload);
    ASSERT_EQ(out.target, random_dest);
  }
}
