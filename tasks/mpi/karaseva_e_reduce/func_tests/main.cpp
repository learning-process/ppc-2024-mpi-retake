...
TEST(karaseva_e_reduce_mpi, test_reduce_large_matrix) {
  ...
  std::vector<int> local_data(N, rank + 1);  // Инициализация данных для каждого процесса
  std::vector<int> reduced_data(N, 0);  // Вектор для результатов редукции
  ...
  // Checking the result on rank 0
  if (rank == 0) {
    std::vector<int> expected_result(N, 0);
    for (int p = 0; p < size; ++p) {
      for (size_t i = 0; i < N; ++i) {
        expected_result[i] += (p + 1);  // Ожидаемая редукция
      }
    }
...
TEST(karaseva_e_reduce_mpi, test_reduce_vs_mpi_reduce) {
  ...
  // Perform MPI_Reduce for comparison
