...
TEST(vasenkov_a_bellman_ford_mpi, test_pipeline_run) {
  ...
  // Create Task
  auto test_task_mpi = std::make_shared<vasenkov_a_bellman_ford_mpi::BellmanFordMPI>(task_data_par);
  ...
  if (world.rank() == 0) {
    ...
    ASSERT_EQ(global_result, expected_distances);
  }
}
