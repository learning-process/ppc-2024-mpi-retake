...
bool sedova_o_multiply_matrices_ccs_seq::TestTaskSequential::RunImpl() {  // NOLINT
  ...
  for (int col_b = 0; col_b < cols_B_; ++col_b) {
    ...
    for (int i = B_col_ptr_[col_b]; i < B_col_ptr_[col_b + 1]; ++i) {  // NOLINT
      int row_b = B_row_ind_[i];                                       // NOLINT
      x[row_b] = i;
    ...
    for (int col_a = 0; col_a < static_cast<int>(At_col_ptr_.size() - 1); ++col_a) {
      double sum = 0.0;
      for (int i = At_col_ptr_[col_a]; i < At_col_ptr_[col_a + 1]; ++i) {  // NOLINT
        int row_a = At_row_ind_[i];                                        // NOLINT
        if (x[row_a] != -1) {
