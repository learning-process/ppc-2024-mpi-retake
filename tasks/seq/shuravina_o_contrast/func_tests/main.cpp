...
TEST(shuravina_o_contrast, test_contrast_stretching_gradient_image) {
  constexpr size_t kSize = 8;
...
TEST(shuravina_o_contrast, test_contrast_stretching_single_value_image) {
  constexpr size_t kSize = 8;

  std::vector<uint8_t> in(kSize * kSize, 64);
  std::vector<uint8_t> out(kSize * kSize, 0);
