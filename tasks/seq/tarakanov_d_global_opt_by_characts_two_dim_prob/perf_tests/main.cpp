...
void runPerfTest(bool isPipeline) {
  ...
  taskDataPar->inputs = {reinterpret_cast<uint8_t*>(area.data()), reinterpret_cast<uint8_t*>(func.data()),
  ...
  auto testClassPar = std::make_shared<tarakanov_d_global_opt_two_dim_prob_seq::GlobalOptSequential>(taskDataPar);
