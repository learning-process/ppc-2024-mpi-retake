...
TEST(konkov_i_LinearHistStretchTest, AllPixelsSameValueSeq) {
  ...
  InitializeImageDataSeq(image_data, image_size, 128);
  konkov_i_linear_hist_stretch::LinearHistogramStretch lht(image_size, image_data);
...
void InitializeImageDataSeq(int* image_data, int image_size, int value) {
  for (int i = 0; i < image_size; ++i) {
    image_data[i] = value;
  }
}

void CheckImageDataSeq(const int* image_data, int image_size, int expected_value) {
  for (int i = 0; i < image_size; ++i) {
    EXPECT_EQ(image_data[i], expected_value);
  }
}

void CheckImageDataRangeSeq(const int* image_data, int image_size, int min_value, int max_value) {
  for (int i = 0; i < image_size; ++i) {
