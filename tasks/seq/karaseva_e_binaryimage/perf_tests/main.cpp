...
TEST(karaseva_e_binaryimage_seq, test_pipeline_run) {
  ...
  task_data_seq->outputs_count.emplace_back(columns);

  // Creating the task
  auto test_task_sequential = std::make_shared<karaseva_e_binaryimage_seq::TestTaskSequential>(task_data_seq);
...
TEST(karaseva_e_binaryimage_seq, test_task_run) {
  const int rows = 1250;
  const int columns = 1250;
  std::vector<int> in(rows * columns);
  std::vector<int> out(rows * columns, -1);
